...
import { mainContractAbi, mainContractAddress, agentContractAbi, agentContractBytecode } from "@/app/abi";
...
async function postHandler(req: Request, res: NextResponse) {
    try {
        ...
        if (!subscriptionRate || isNaN(Number(subscriptionRate)) || Number(subscriptionRate) <= 0) {
            return NextResponse.json({
        ...
        if (!PRIVATE_KEY || !RPC_URL) {
            console.error("Missing environment variables:", { PRIVATE_KEY: !!PRIVATE_KEY, RPC_URL: !!RPC_URL });
            return NextResponse.json({
        ...
        const subscriptionRateWei = ethers.parseUnits(subscriptionRate.toString(), "wei");

        // Validate constructor parameters
        if (!ethers.isAddress(userAddress) || !ethers.isAddress(mainContractAddress) || subscriptionRateWei <= 0) {
        ...
        if (deploymentTx) {
            ...
            if (receipt?.contractAddress && receipt.contractAddress !== agentContractAddress) {
                ...
                if (receiptCode !== '0x') {
                    ...
                    const agentContract = new ethers.Contract(receiptAddress, agentContractAbi, provider);
        ...
        if (deployedCode === '0x') {
            ...
            if (deployedCode === '0x') {
                ...
                deployedCode = await freshProvider.getCode(agentContractAddress);
        ...
        try {
            const agentContract = new ethers.Contract(agentContractAddress, agentContractAbi, provider);
            await agentContract.user();
            await agentContract.mainContract();
            await agentContract.subscriptionRate();
        } catch (error) {
